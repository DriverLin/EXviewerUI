    ...
    def g_data_from_pageHtml(self, gid, token):
        ...
        try:
            ...
            if favoText != " Add to Favorites":
                ...
                postion = style.split("background-position:0px ")[1].split("px;")[0]
                favoNum = ["-2","-21","-40","-59","-78","-97","-116", "-135","-154","-173"].index(postion)
                favo = favoNum
            ...
            g_data = {
                ...
                "extended": {
                    ...
                    "download": self.getDownloadExtendvalue(gid),#-2:未下载 -1:下载中 0-n:下载完成数量
                },
            }
            ...
            infos.append(
                {
                    ...
                    "lang": lang,
                    "pages": pages,
                    "favo": 999 if favo else -1,#999代表收藏 但是不确定收藏夹编号 (其实可以知道名称)
                    "download": self.getDownloadExtendvalue(gid),
                }
            )
        return infos
